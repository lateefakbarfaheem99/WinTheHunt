...
// Dave Hofer added this NewRelic code to tell NewRelic that this is a batch file.
    if(extension_loaded('newrelic')) { 
       newrelic_background_job (); 
    }
// End Dave Hofer NewRelic Change
...
switch($_GET['type']){
	...
	case 'extend':
		...
		while (time() < $expireTime) {
			...
			$autobid_time1 				= rand(1,4); //range 1 
			$autobid_time2 				= rand(3,5); // range 2
			$autobid_time3 				= rand(1,3); // range 3
			$select_autobid_time 		= rand(1,3); // choosing range randomly
			$autobid_time_var			= 'autobid_time';
			$autobid_time_range = $autobid_time_var.$select_autobid_time;
			$autobid_time = $$autobid_time_range;
	...
	case 'close':
		...
		while (time() < $expireTime) {
			...
			$date_now = date('Y-m-d H:i:s');$strtotime_extnd = (strtotime($date_now) - 1); //time  
			$newExtendeddate = date('Y-m-d H:i:s',$strtotime_extnd);
			$sql_row = "SELECT * FROM auctions WHERE end_time <= '".$newExtendeddate."' AND closed = 0 AND active = 1 AND deleted=0 ";
			$sql = mysql_query($sql_row);
