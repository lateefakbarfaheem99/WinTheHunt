...
foreach($data as $key => $value){
	...
	if($result['Auction']['closed']){
		$result['Auction']['end_time_string'] = 'Ended';
	}
	
	
	unset($result['Auction']['closed']);  unset($result['Product']);
	unset($result['LastBid']['id']);unset($result['LastBid']['description']);unset($result['LastBid']['created']);unset($result['LastBid']['user_id']);
	unset($result['Auction']['product_id']);unset($result['Auction']['start_time']);unset($result['Auction']['end_time']);unset($result['Auction']['serverTimestamp']);
	unset($result['Auction']['price_increment']);unset($result['Auction']['closes_on']);unset($result['Auction']['serverTimeString']);
