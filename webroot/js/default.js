$(document).ready(function(){
	...
	function getNewAuctionItem()
	{
		sfp = "type=1";

		$.ajax({
		  url: "auctions/ajax_get_next_auction",
		  cache: false,
		  data: sfp,
		  type: "POST",
		  async: true,
		  ...
		  success: function(data, textStatus, xhr ){
				...
				$('.horizontal-bid-list').append(data);
				get_auctions();
	...
	function getNewVerticalAuctionItem()
	{
		sfp = "type=1";

		$.ajax({
		  url: "auctions/ajax_get_next_vertical_auction",
		  cache: false,
		  data: sfp,
		  type: "POST",
		  async: true,
		  ...
		  success: function(data, textStatus, xhr ){
				...
				$('#vertical-bid-list').append(data);
				get_auctions();
			
			}
		});

		$('#vertical-bid-list li').first().remove();

		$("#vertical-bid-list li").each(function (index, value) {  
		  if((index%2)==0)
				$(this).addClass('live_bid_lightgray');
		  else
				$(this).removeClass('live_bid_lightgray');
	    	});  
            ...
            $.ajax({
                ...
                success: function(data){
                    ...
                    $.each(data, function(i, item){
						...
						if(item.Auction.time_left < 1 && item.Auction.end_time_string != 'Ended'){
                            auctionObjects[item.Auction.element]['countdown'].html('Verify Bids');

                        }
						
						if(item.Auction.end_time_string == 'Ended'){
							...
							if(delayCount == 10)
							{
								...
								getNewAuctionItem();							
								auctionObjects[item.Auction.element].remove();															
								getNewVerticalAuctionItem(); 
