...
class AuctionsController extends AppController {
	...
	var $helpers = array('Epayment', 'Html', 'Text');

	function beforeFilter(){
		parent::beforeFilter();

		if(!empty($this->Auth)) {
			$this->Auth->allow('test', 'popup', 'index', 'view', 'live', 'home', 'future', 'closed', 'winners', 'getcount', 'getstatus', 'ipn', 'latestwinner', 'gettickerauctions', 'endingsoon', 'getfutureauctions', 'getfeatured', 'getauctions', 'getwinners', 'creditcard', 'credits', 'getendedlist', 'free', 'search', 'timeout', 'ajax_get_next_auction', 'ajax_get_next_vertical_auction');
		}
	}
	...
	function view($id = null) {
		...
		$auctions_end_soon=$this->Auction->find('all', array(	'conditions'=>array(	'Auction.end_time > '=>$this->getEndTime(),
																'Auction.start_time <='=>date('Y-m-d H:i:s'),
																'Auction.active'=>1,
																'Auction.id NOT'=>$auction['Auction']['id']
																),
												'contain'=>array('Product' => array('Image' => 'ImageDefault'), 'Leader'),
												'fields'=>array('Auction.*','Product.*','Leader.username'),
												'limit'=>'3',
	...
	function ajax_get_next_auction($type=1)
	{
		
		if($type == 1)	// ending soon auctions at hompepage.
		{
			...
			$row_count = count($auctions_end_soon);
			if($row_count < Configure::read('App.homeEndingLimit'))
				return;

			$auction = $auctions_end_soon[$row_count-1];

			Cache::write('auctions_end_soon', $auctions_end_soon, 'minute');
		}
		elseif($type=3)		//ending soon auctions in view auction page.
		{
			$auctions_end_soon=$this->Auction->find('all', array(	'conditions'=>array(	'Auction.end_time > '=>$this->getEndTime(),
																'Auction.start_time <='=>date('Y-m-d H:i:s'),
																'Auction.active'=>1,
	...
	function ajax_get_next_vertical_auction()
	{
		...
		$row_count = count($auctions_live);
		if($row_count < Configure::read('App.homeFeaturedLimit'))
			return;
		
		$auction = $auctions_live[$row_count-1];
