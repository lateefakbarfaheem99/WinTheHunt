            ...
            switch($model){
                case 'auction':
                    if(!empty($data['t_status']) && $data['t_status'] == 2){
						...
						$this->add_bids($user_id, '', $auction, 0 );
...
//			$amount 	 = number_format($amount, 2);
    ...
    function payment_network_ipn(){
	 ...
	 //function for automatically bids creditting.
	function add_bids($user_id = null, $description = 'test', $auction = null , $addpoints = 1){  
        if(!empty($user_id) && $auction['Product']['is_bidpack'] == 1 ){ 
        	$user = $this->User->find('first', array('conditions' => array('User.id' => $user_id), 'contain' => ''));
			$credit = $auction['Product']['no_of_bids'];
			$description = 'Won auction bids';
            if($this->appConfigurations['simpleBids'] == true) { 
				
				if($credit > 0) { 
					$user['User']['bid_balance'] += $credit;
					if($addpoints == 1){$user['User']['grab_points'] += $credit; }
				} else { 
					$user['User']['bid_balance'] -= $debit;
				} 
				$this->User->save($user, false);
				//echo 'veriable : '.$user['User']['bid_balance'] ;exit;
				$bid['Bid']['user_id']     = $user_id;
	            $bid['Bid']['description'] = $description;
	            $bid['Bid']['credit']      = $credit;
	            $bid['Bid']['debit']       = $debit;
		        ...
		        $account['Account']['bids']    = $credit;
		        $account['Account']['price']   = $price; 
           		//mysql_query("INSERT INTO grabzones set user_id = '". $user_id ."', bp_no_of_bids = '". $credit ."' , bp_price = '". $price ."', created = '". date('Y-m-d H:i:s') ."', modified = '". date('Y-m-d H:i:s') ."', record_type = 5   ");
		        //$this->Account->create();
		        ...
		        $account['Account']['bids']    = $credit;
		        $account['Account']['price']   = $auction['Auction']['price']; 
            	$this->Account->save($account);
            	
				$bid['Bid']['user_id']     = $user_id;
	            $bid['Bid']['description'] = $description;
	            $bid['Bid']['credit']      = $auction['Product']['no_of_bids'];
	            $bid['Bid']['debit']       = 0;
	            $this->Bid->create();
	            return $this->Bid->save($bid);
