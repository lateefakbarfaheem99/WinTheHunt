...
function bid($data = array(), $autobid = false, $bid_description = null) {
	...
	if(!empty($auction)){
		...
		// tired daily 
		if(!empty($config['App']['limits_t_daily']['active'])) {
			$limits_exceeded = limitsCanBid_t_d($data['auction_id'], $data['user_id'], $auction['product_id'], $autobid);
			if($limits_exceeded == false) {
				logIt($auction['id'].' '.$data['user_id'].' - cantBid: exceeded bidding limit');
				$message = __('You cannot bid on this auction as your have exceeded your daily limit.', true);
				$canBid = false;
			}
		}
		// tired weekly 
		if(!empty($config['App']['limits_t_weekly']['active'])) {
			$limits_exceeded = limitsCanBid_t_w($data['auction_id'], $data['user_id'], $auction['product_id'], $autobid);
			if($limits_exceeded == false) {
				logIt($auction['id'].' '.$data['user_id'].' - cantBid: exceeded bidding limit');
				$message = __('You cannot bid on this auction as your have exceeded your weekly limit.', true);
				$canBid = false;
			}
		}
		// tired monthly 
		if(!empty($config['App']['limits_t_monthly']['active'])) {
			$limits_exceeded = limitsCanBid_t_m($data['auction_id'], $data['user_id'], $auction['product_id'], $autobid);
			if($limits_exceeded == false) {
				logIt($auction['id'].' '.$data['user_id'].' - cantBid: exceeded bidding limit');
				$message = __('You cannot bid on this auction as your have exceeded your monthly limit.', true);
				$canBid = false;
			}
		}
		...
		if(!empty($auction['closed']) && $autobid == false && !$data['bid_butler']) {
...
function limitsCanBid_t_d($auction_id = null, $user_id = null, $product_id = null, $autobidder = false) {
	global $config;
	...
	$expiry_date = date('Y-m-d H:i:s', time() - ($config['App']['limits_t_daily']['expiry'] * 24 * 60 * 60));

	$total_res = mysql_fetch_array(mysql_query("SELECT 	SUM(p.rrp) as total FROM `auctions` a LEFT JOIN products p ON a.product_id = p.id WHERE a.`leader_id` = '$user_id'  AND a.end_time > '$expiry_date'"), MYSQL_ASSOC);
	$total_res = $total_res['total'];
	
	if($total >= $config['App']['limits_t_daily']['limit']) {
		return false;
	}
...
function limitsCanBid_t_w($auction_id = null, $user_id = null, $product_id = null, $autobidder = false) {
	global $config;
	...
	$expiry_date = date('Y-m-d H:i:s', time() - ($config['App']['limits_t_weekly']['expiry'] * 24 * 60 * 60));

	$total_res = mysql_fetch_array(mysql_query("SELECT 	SUM(p.rrp) as total FROM `auctions` a LEFT JOIN products p ON a.product_id = p.id WHERE a.`leader_id` = '$user_id'  AND a.end_time > '$expiry_date'"), MYSQL_ASSOC);
	$total_res = $total_res['total'];
	
	if($total >= $config['App']['limits_t_weekly']['limit']) {
		return false;
	}
...
function limitsCanBid_t_m($auction_id = null, $user_id = null, $product_id = null, $autobidder = false) {
	global $config;
	...
	$expiry_date = date('Y-m-d H:i:s', time() - ($config['App']['limits_t_monthly']['expiry'] * 24 * 60 * 60));

	$total_res = mysql_fetch_array(mysql_query("SELECT 	SUM(p.rrp) as total FROM `auctions` a LEFT JOIN products p ON a.product_id = p.id WHERE a.`leader_id` = '$user_id'  AND a.end_time > '$expiry_date'"), MYSQL_ASSOC);
	$total_res = $total_res['total'];
	
	if($total >= $config['App']['limits_t_monthly']['limit']) {
		return false;
	}
